...
import { Plus, Trash2, Edit, Upload, Users, MapPin, DollarSign, Calendar } from 'lucide-react';
              ...
              <tbody>
                {bookings.length > 0 ? bookings.map((b, index) => (
                  <tr key={b.id || index} style={{ borderBottom: '1px solid rgba(148, 163, 184, 0.1)' }}>
                    <td style={{ color: '#f8fafc', padding: '1rem', verticalAlign: 'middle', fontWeight: '600' }}>{b.customer_name || b.name}</td>
                    <td style={{ color: '#94a3b8', padding: '1rem', verticalAlign: 'middle' }}>{b.customer_phone || b.phone}</td>
                    <td style={{ color: '#f8fafc', padding: '1rem', verticalAlign: 'middle' }}>{b.stadium_name || b.stadium}</td>
